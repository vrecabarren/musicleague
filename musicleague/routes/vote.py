...
def vote(league_id, submission_period_id):
    ...
    league = select_league(league_id)
    app.logger.info('League loaded after %ss', timer() - start)
    ...
    if not submission_period.accepting_votes:
        app.logger.warning('No longer accepting votes - redirecting after %ss', timer() - start)
        return redirect(request.referrer)

    try:
        app.logger.info('Loading votes from form submission after %ss', timer() - start)
        votes = json.loads(request.form.get('votes'))
        app.logger.info('Votes loaded after %ss', timer() - start)
    except Exception:
    ...
    app.logger.info('Removing zero values after %ss', timer() - start)
    votes = {k: v for k, v in votes.iteritems() if v}
    app.logger.info('Removed zero values after %ss', timer() - start)

    # Process votes
    app.logger.info('Creating/updating vote after %ss', timer() - start)
    vote = create_or_update_vote(votes, submission_period, league, g.user)
    app.logger.info('Vote created/updated after %ss', timer() - start)

    # If someone besides owner is voting, notify the owner
    if g.user.id != league.owner.id:
        app.logger.info('Notifying owner of user vote after %ss', timer() - start)
        owner_user_voted_notification(vote)

    app.logger.info('Checking remaining voters after %ss', timer() - start)
    remaining = submission_period.have_not_voted
    if not remaining:
        app.logger.info('Completing submission period after %ss', timer() - start)
        complete_submission_period.delay(submission_period.id)

    elif vote.count < 2 and len(remaining) == 1:
        app.logger.info('Notifying final voter after %ss', timer() - start)
        last_user = remaining[0]
        user_last_to_vote_notification(last_user, submission_period)

    app.logger.info('Vote request complete after %ss', timer() - start)
    return redirect(url_for('view_league', league_id=league_id))
